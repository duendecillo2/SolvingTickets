...
const Administracion = () => {
  ...
  const handleAction = (ticket) => {
    ...
    setResponseMessage(''); // Limpia el mensaje previo
  ...
  const handleSubmitResponse = async () => {
    try {
      ...
      await axios.post(`http://localhost:8000/api/tickets/${selectedTicket.id}/responder/`, { respuesta: responseMessage }, {
          headers: {
  ...
  // Columnas definidas dentro del componente para usar `handleAction`
  const columns = [
    ...
    <ThemeProvider theme={theme}>
      ...
      <Modal open={isModalOpen} onClose={handleCloseModal}>
        ...
        >
          ...
          <TextField
            value={selectedTicket?.mensaje || ''} // Uso de `?` para evitar errores
            InputProps={{
